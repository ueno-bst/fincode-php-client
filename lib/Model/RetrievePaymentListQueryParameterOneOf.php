 ...
 */
class RetrievePaymentListQueryParameterOneOf implements ModelInterface, \ArrayAccess, \JsonSerializable
{
    public const DISCRIMINATOR = null;
    /**
     * The original name of the model.
     */
    protected static string $openAPIModelName = 'retrievePaymentList_query_parameter_oneOf';
    /**
    ...
    public function setKeyword(mixed $keyword): static
    {
        ...
        if (!is_null($keyword) && mb_strlen($keyword) > 100) {
            throw new \InvalidArgumentException('invalid length for $keyword when calling RetrievePaymentListQueryParameterOneOf., must be smaller than or equal to 100.');
        }
        if (!is_null($keyword) && mb_strlen($keyword) < 1) {
            throw new \InvalidArgumentException('invalid length for $keyword when calling RetrievePaymentListQueryParameterOneOf., must be bigger than or equal to 1.');
        }
    ...
    public function setCustomerId(mixed $customerId): static
    {
        ...
        if (mb_strlen($customerId) > 60) {
            throw new \InvalidArgumentException('invalid length for $customerId when calling RetrievePaymentListQueryParameterOneOf., must be smaller than or equal to 60.');
        }
        if (mb_strlen($customerId) < 1) {
            throw new \InvalidArgumentException('invalid length for $customerId when calling RetrievePaymentListQueryParameterOneOf., must be bigger than or equal to 1.');
        }
     ...
     * Sets status.
     *
     * @param PaymentStatus $status 決済ステータス\\ 指定したステータスの決済情報を取得します。カンマ区切りにすることで複数指定（OR検索）できます。  - `UNPROCESSED`: 未処理 - `CHECKED`: 有効性チェック済み - `AUTHORIZED`: 仮売上 - `CAPTURED`: 売上確定 - `CANCELED`: キャンセル - `AUTHENTICATED`: 未処理（3Dセキュア認証待ち）
     */
     ...
     * Sets sort.
     *
     * @param string|null $sort ソートする項目と順序を指定します。\\ 例： `?sort=updated␣desc,created␣asc`\\ \\ ソート可能な項目  - `status`: ステータス - `process_date`: 処理日時 - `total_amount`: 合計金額 - `auth_max_date`: 仮売上有効期限 - `created`: 作成日時 - `updated`: 更新日時
     */
