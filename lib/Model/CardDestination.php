...
class CardDestination implements ModelInterface, \ArrayAccess, \JsonSerializable
{
     ...
     */
    protected static array $openAPITypes = ['contractCardInfoMasterId' => 'int', 'examinationMasterId' => 'float', 'destination' => 'string', 'destinationType' => 'float'];
    /**
    ...
    {
        $invalidProperties = [];
        $allowedValues = $this->getExaminationMasterIdAllowableValues();
        if (!is_null($this->container['examinationMasterId']) && !in_array($this->container['examinationMasterId'], $allowedValues, true)) {
            $invalidProperties[] = sprintf("invalid value '%s' for 'examinationMasterId', must be one of '%s'", $this->container['examinationMasterId'], implode("', '", $allowedValues));
        }
     ...
     *
     * @param float|null $examinationMasterId 審査種別マスタID   - `1`：  カード決済（VISA / Mastercard, UCカード社） - `2`：  カード決済（JCB / American Express / Diners Club / Discover） - `3`：  カード決済（VISA / Mastercard, トヨタファイナンス社）
     */
    public function setExaminationMasterId(mixed $examinationMasterId): static
    {
        ...
        $allowedValues = $this->getExaminationMasterIdAllowableValues();
        if (!is_null($examinationMasterId) && !in_array($examinationMasterId, $allowedValues, true)) {
            throw new \InvalidArgumentException(sprintf("Invalid value '%s' for 'examinationMasterId', must be one of '%s'", $examinationMasterId, implode("', '", $allowedValues)));
        }
